...
//根中间件
func RootMidware(ctx *modules.Context) {
	ctx.Data["site_name"] = conf.Server.SiteName
}
...
func NeedIsGamer(ctx *modules.Context) {
	...
	if agent_id == 0 {
		ctx.Error("您的微信账号还未与"+conf.Server.SiteName+"游戏关联，请先使用微信登录游戏再访问本页面。","",0)
		return
	}
}
...
func NeedIsAgent(ctx *modules.Context)  {
	...
	agent_id := agentModel.GetUserIdByUnionId(wx_info.UnionId)
	if agent_id == 0 {
		ctx.Error("您的微信账号还未与"+ conf.Server.SiteName +"游戏关联，请先使用微信登录游戏再刷新本页面。","",0)
		return
	}
	//验证是否为代理商
