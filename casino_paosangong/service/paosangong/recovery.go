...
func (room *Room) GetSnapShotIdList() *ddproto.NiuniuSrvDeskSnapshotIdIndex {
	desk_id_list_key := consts.RKEY_NIUNIU_SNAPSHOT_ID_LIST
	list := redisUtils.GetObj(desk_id_list_key, &ddproto.NiuniuSrvDeskSnapshotIdIndex{})
	if list == nil {
		return &ddproto.NiuniuSrvDeskSnapshotIdIndex{
			DeskId: []int32{},
		}
	}
	return list.(*ddproto.NiuniuSrvDeskSnapshotIdIndex)
}
...
func (desk *Desk) RemoveSnapShot() error {
	log.T("开始删除牌桌pwd:%v id:%v快照.", desk.GetDeskNumber(), desk.GetDeskId())
	if desk == nil {
		return errors.New("desk nil.")
	}
	...
	for i, ex_desk_id := range list.DeskId {
		if ex_desk_id == desk.GetDeskId() {
			...
			//删除快照
			desk_snap_key := redisUtils.K_STRING(consts.RKEY_NIUNIU_SNAPSHOT_DATA, numUtils.Int2String2(desk.GetDeskId()))
	...
	log.E("删除牌桌pwd:%v id:%v快照失败,未找到该快照。", desk.GetDeskNumber(), desk.GetDeskId())
	return nil
}
...
func (desk *Desk) WipeSnapShot() error {
	...
	go func() error {
		...
		new_snapshot := &ddproto.NiuniuSrvDeskSnapshot{
			DeskState: desk.NiuniuSrvDesk,
			Users: []*ddproto.NiuniuSrvUser{},
		}

		for _,u := range desk.Users {
			new_snapshot.Users = append(new_snapshot.Users, u.NiuniuSrvUser)
		}
...
func (room *Room) RecoveryDeskSnapShot(desk_id int32) {
	...
	desk_snap_key := redisUtils.K_STRING(consts.RKEY_NIUNIU_SNAPSHOT_DATA, numUtils.Int2String2(desk_id))

	redis_snap_shot := redisUtils.GetObj(desk_snap_key, &ddproto.NiuniuSrvDeskSnapshot{})
	...
	snap_shot := redis_snap_shot.(*ddproto.NiuniuSrvDeskSnapshot)
	...
	new_desk := &Desk{
		Room: room,
		NiuniuSrvDesk: snap_shot.DeskState,
		Users: []*User{},
	}
...
func (desk *Desk) OnAfterRecoveryDo() {
	...
	if desk.GetIsCoinRoom() && desk.GetStatus() == ddproto.NiuniuEnumDeskState_NIU_DESK_STATUS_WAIT_JIABEI {
	...
	for _,u := range desk.Users {
		...
		case ddproto.NiuniuEnumDeskState_NIU_DESK_STATUS_WAIT_READY:
	...
	if desk.GetIsCoinRoom() {
		//强制将最大抢庄数改为4，防止数据恢复到旧数据
		desk.DeskOption.MaxQzScore = proto.Int64(4)
	}
