...
func (l *MjPBLianzi) AddPai(pai *MJPai) {
	l.pais = append(l.pais, pai)
	//做初始化
	l.flower = pai.GetFlower()
	var paiList MjPaiList = l.pais
	sort.Sort(paiList)
...
//删除一个list
func (p *MjParser) rmPaiList(list []*MJPai) {
...
//删除temp中指定的牌
func (p *MjParser) rmTempPai(rmp *MJPai) {
	index := -1
...
func (p *MjParser) GetDanPaiByCountInex(index int32) *MJPai {
...
//判断单排是否可以加入到链子中
func (p *MjParser) Dan2Lian(pai *MJPai) bool {
...
//得到推荐打的牌
func (p *MjParser) GetOutPai() *MJPai {
	//1,找缺牌
	if len(p.QUE) > 0 {
		return p.QUE[0]
	}
...
//初始化
func NewDefaultMjParser(handpai *MJHandPai) *MjParser {
	//log.T("开始初始化解析器.")
	//初始化所有牌
	p := new(MjParser)
...
func GetOutPai(handpai *MJHandPai) (ret *MJPai) {
...
func GetDingQUe(handpai *MJHandPai) (ret int32) {
	defer func() { // 必须要先声明defer，否则不能捕获到panic异常
		if err := recover(); err != nil {
			log.E("解析麻将的时候出现了错误")
			fmt.Println(err) // 这里的err其实就是panic传入的内容，55
