...
func (t *ThDesk) InitUserStatus() error {
	...
	for i := 0; i < len(t.Users); i++ {
		...
		if !t.IsUserRoomCoinEnough(u) || u.IsBreak || u.IsLeave {
...
func (t *ThDesk) getRankByUserId(user *ThUser) int32 {
	if t.IsChampionship() {
		room := GetCSTHroom(user.MatchId)
		if room != nil {
			return room.GetRankByuserId(user.UserId)
		} else {
			log.T("查询用户userId【%v】,nickName[%v]排名的时候，通过 matchId [%v] 查询room出错...", user.UserId, user.NickName, user.MatchId)
			return 0
		}
	} else {
		return 0
...
func (t *ThDesk) getCanRebuyByUserId(user *ThUser) bool {
	if t.IsFriend() {
		...
		log.T("判断user【%v】锦标赛是否能重购买t.IsUserRoomCoinEnough(user)[%v],user.RebuyCount [%v],t.RebuyCountLimit[%v],t.blindLevel[%v],t.RebuyBlindLevelLimit[%v]",
			user.UserId, t.IsUserRoomCoinEnough(user), user.RebuyCount, t.RebuyCountLimit, t.blindLevel, t.RebuyBlindLevelLimit)
...
func (t *ThDesk) CalcThcardsWin() error {
	...
	for i := 0; i < len(t.Users); i++ {
		u := t.Users[i]
		if u != nil &&
		u.IsGaming()  && pokerService.ThCompare(userWin.thCards, u.thCards) == pokerService.THPOKER_COMPARE_EQUALS {
...
func (t *ThDesk) Less(u1, u2 *ThUser) bool {
	...
	if u1 == nil || !u1.IsGaming() {
		return true
	}

	if u2 == nil || !u2.IsGaming() {
		return false
	}
	...
	for i := 0; i < len(t.Users); i++ {
		...
		if u != nil {
			...
			if (u.IsAllIn() || u.IsBetting()) && allinOrBettingCount >= 2 {
				...
				u.CloseCheck = TH_USER_CLOSECHECK_GAMING        //未结算
				u.IsShowCard = true
			} else if u.IsFold() {
				u.CloseCheck = TH_USER_CLOSECHECK_CLOSE        //已结算
			}
		}
	}
	return nil
...
func (t *ThDesk) calcUserWinAmount() error {
	...
	if bwinCount != 0 {
		...
		for i := 0; i < len(t.Users); i++ {
			...
			if u != nil && u.IsGaming() && u.thCards.IsWin {
				//
				//对这个用户做结算...
...
func (t *ThDesk) CSRebuy(userId uint32) error {
	...
	user.WaitRebuyFlag = false
	user.Update2redis()
	user.AddRebuyCount()                //增加重购的次数...
