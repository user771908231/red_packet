...
func HandlerGameEnterMatch(m *bbproto.Game_EnterMatch, a gate.Agent) error {
	//定义需要的参数
	var userId uint32 = 10007                                //获取到用户的userId
	...
	*result.NInitDelayTime   = int32(room.TH_TIMEOUT_DURATION)
	...
	for i := 0; i < len(mydesk.Users);i++  {
		...
		if u != nil {
			...
			result.Handcard = append(result.Handcard,getHandCard(u)...)
...
func isEnable(u *room.ThUser) int32{
	if u.Status == room.TH_USER_STATUS_BETING {	//游戏中
		return 1
	}else{
		return 0
	}
...
func isFold(u *room.ThUser) int32{
	if u.Status == room.TH_USER_STATUS_FOLDED {	//游戏中
		return 1
	}else{
		return 0
	}
...
//解析手牌
func getHandCard(u *room.ThUser) []*bbproto.Game_CardInfo{
...
func deskStatus2OG(desk *room.ThDesk) int32{
	...
	if status == room.TH_DESK_STATUS_STOP{				//没有开始
