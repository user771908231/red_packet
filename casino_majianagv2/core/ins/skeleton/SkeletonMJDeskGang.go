...
func (d *SkeletonMJDesk) DoGangBill(info *majiang.GangPaiInfo) {
	gangType := info.GetGangType()
	gangUser := d.GetSkeletonMJUserById(info.GetGetUserId())
	gangPai := info.GetPai()

	if gangType == majiang.GANG_TYPE_AN {
		//处理暗杠的账单
		score := d.GetMJConfig().BaseValue * 2 //暗杠的分数
		for _, ou := range d.GetSkeletonMJUsers() {
			//不为nil 并且不是本人，并且没有胡牌
		...
		shuUser := d.GetSkeletonMJUserById(info.GetSendUserId())
