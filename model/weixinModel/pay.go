...
//统一下单
func GetUnifiedOrderResponse(tradeNo string, totalFee float64, detail string, user_ip string, user_openid string) (*pay.UnifiedOrderResponse, error) {
	...
	request := pay.UnifiedOrderRequest{
		...
		NotifyURL: WX_NOTIFY_URL, // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
